...
class Detector(object):
    ...
    def detect(self, prob_threshold=0.6, area_threshold=100, proposal_threshold=0.05, vis=False):
        total_dts = 0
        ...
        for i, data in enumerate(loader):
            ...
            if self.args.reuse_encoder_cache:
                proposals, memory, spatial_shapes, level_start_index, valid_ratios, mask_flatten = \
                    self.detr.inference_single_image_proposals_light_light(
                        cur_img, (seq_h, seq_w), score_threshold=proposal_threshold)
                res = self.detr.inference_single_image_light_light(
                    cur_img,
                    (seq_h, seq_w),
                    track_instances,
                    proposals,
                ...
                proposals = self.detr.inference_single_image_proposals(
                    cur_img, (seq_h, seq_w), score_threshold=proposal_threshold)
...
if __name__ == '__main__':
    ...
    if not args.output_dir:
        raise ValueError("--output_dir must be provided. Results are saved under --output_dir/--exp_name.")
    if args.output_dir:
        Path(args.output_dir).mkdir(parents=True, exist_ok=True)

    checkpoint = load_torch_checkpoint(args.resume, map_location='cpu', weights_only=False)
    args = apply_checkpoint_model_args(args, checkpoint, context='eval_bft')
    ...
    output_root = os.path.join(args.output_dir, args.exp_name)
    os.makedirs(output_root, exist_ok=True)
    print(f"[eval_bft] Output directory: {output_root}")
    print(f"[eval_bft] Total sequences before split: {len(seq_nums)}")
    print(f"[eval_bft] RLAUNCH_REPLICA={rank}, RLAUNCH_REPLICA_TOTAL={ws}, assigned sequences={len(vids)}")
    if len(vids) == 0:
        print("[eval_bft] No sequences assigned to this replica. Nothing will be written.")
