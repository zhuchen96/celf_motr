...
class Detector(object):
    ...
    @staticmethod
    def filter_dt_by_score_v2(dt_instances: Instances, prob_threshold: float) -> Instances:
    ...
    def detect(self, prob_threshold=0.5, area_threshold=100, proposal_threshold=0.05, vis=False):
        ...
        for i, (cur_img, ori_img) in enumerate(tqdm(loader)):
            ...
            proposals = self.detr.inference_single_image_proposals(cur_img.cuda().float(), (seq_h, seq_w), score_threshold=proposal_threshold)
            res = self.detr.inference_single_image(cur_img.cuda().float(), (seq_h, seq_w), track_instances, proposals)
            track_instances = res['track_instances']

            dt_instances = deepcopy(track_instances)

            # filter det instances by score.
            dt_instances = self.filter_dt_by_score_v2(dt_instances, prob_threshold)
...
if __name__ == '__main__':
    ...
    args = apply_checkpoint_model_args(args, checkpoint, context='submit_mot17')
    ...
    for seq_num in seq_nums:
        ...
        det.detect(prob_threshold=args.score_threshold, proposal_threshold=args.proposal_threshold)
    ...
    seq_nums = os.listdir('/images/SegmentationDistillation/data/MOT17/images/train')
    accs = []
    seqs = []
    for seq_num in seq_nums:
