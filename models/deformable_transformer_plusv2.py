...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 extra_track_attn=False, memory_bank=False, shared_decoder=False):
        super().__init__()
        ...
        self.decoder = DeformableTransformerDecoder(
            decoder_layer, num_decoder_layers, return_intermediate_dec, shared_decoder=shared_decoder
        )
    ...
    def forward_light_proposal(self, srcs, masks, pos_embeds, query_embed=None, ref_pts=None, mem_bank=None, mem_bank_pad_mask=None, attn_mask=None):
        assert self.two_stage or query_embed is not None
        ...
        if self.two_stage:
            return hs, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact
        return hs, init_reference_out, inter_references_out, None, None, memory, spatial_shapes, level_start_index, valid_ratios, mask_flatten
    
    def forward_light_tracking(self, srcs, masks, pos_embeds, query_embed, ref_pts, memory,
                               spatial_shapes, level_start_index, valid_ratios, mask_flatten,
                               mem_bank=None, mem_bank_pad_mask=None, attn_mask=None):
        #assert self.two_stage or query_embed is not None
        #attn_mask=None
        """
        # prepare input for encoder
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, shared_decoder=False):
        super().__init__()
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid in range(self.num_layers):
            layer = self.layers[0] if self.shared_decoder else self.layers[lid]
...
def build_deforamble_transformer(args):
    return DeformableTransformer(
        ...
        shared_decoder=getattr(args, 'shared_decoder', False),
    )
