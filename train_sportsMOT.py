...
def get_args_parser():
    ...
    parser.add_argument('--num_queries_detect', default=300, type=int,
                        help="Number of query slots")    
    ...
    parser.add_argument('--lambda_detect', type=float, default=1.0)
    return parser
...
def main(args):
    ...
    if args.sgd:
        ...
        optimizer = torch.optim.AdamW(param_dicts, lr=args.lr,
                                      weight_decay=args.weight_decay)
    if args.lr_drop_epochs is not None:
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=args.lr_drop_epochs)
    else:
    ...
    if args.resume:
        if args.resume.startswith('https'):
            ...
            # Keep resume model loading behavior consistent with eval_sportsmot.py
            # to tolerate architecture/shape differences.
            checkpoint = load_torch_checkpoint(args.resume, map_location='cpu', weights_only=False)
            model_without_ddp = load_model(model_without_ddp, args.resume)

        # Always treat --resume as weight initialization only.
        # Optimizer/scheduler/epoch states are intentionally ignored.
        if not args.eval and ('optimizer' in checkpoint or 'lr_scheduler' in checkpoint or 'epoch' in checkpoint):
            print('Info: --resume loads model weights only; optimizer/lr_scheduler/epoch states are ignored.')
        args.start_epoch = 0
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_mot_self_proposal(
            model, criterion, criterion_detect, data_loader_train, optimizer, args.score_threshold, device, epoch, args.clip_max_norm, args.accum_iter, args.lambda_detect)
        lr_scheduler.step()
        if args.output_dir:
